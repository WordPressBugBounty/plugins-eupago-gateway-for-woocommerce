...
?>
    ...
    <script>
        ...
        function eupago_admin_ajax(action, data, successCallback) {
            data.action = action;
            data.security = '<?php echo wp_create_nonce("eupago_settings_nonce"); ?>';
            ...
            $.ajax({
                ...
                error: function(jqXHR, textStatus, errorThrown) {
                    ...
                    alert("An error occurred during the request (Connection error).");
                }
            });
        }
        ...
        document.getElementById("updateCallback").addEventListener("click", function(event) {
            ...
            const data = {
                ...
                callback_api_key: document.getElementById("api_key").value,
                callback_url: "<?php echo esc_js($callback_url); ?>"
            };

            eupago_admin_ajax('eupago_update_callback', data, function(response) {
                if (response.transactionStatus === "Success") {
                    $('#syncEupagoChannel').click();
                    alert(response.message);
                } else {
                    alert(response.text || "Unexpected response.");
                }
            });
        });
        ...
        document.getElementById("syncEupagoChannel").addEventListener("click", function(event) {
            ...
            eupago_admin_ajax('eupago_sync_channel', data, function(response) {
                if (response.transactionStatus === "Success") {
