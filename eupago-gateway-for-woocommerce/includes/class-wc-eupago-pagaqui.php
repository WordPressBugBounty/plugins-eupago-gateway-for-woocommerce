 ...
 * @since 0.1
 */
if (!class_exists('WC_Eupago_Pagaqui')) {
  class WC_EuPago_Pagaqui extends WC_Payment_Gateway
  {
    ...
    protected $sms_payment_hold_pagaqui;
    protected $sms_payment_confirmation_pagaqui;
    protected $client;
    ...
    public function __construct()
    {
      ...
      $this->sms_payment_hold_pagaqui = $this->get_option('sms_payment_hold_pagaqui');
      $this->sms_payment_confirmation_pagaqui = $this->get_option('sms_payment_confirmation_pagaqui');
    ...
    function init_form_fields()
    {
      ...
      $title_pagaqui = __('Title', 'eupago-gateway-for-woocommerce');
      ...
      $pagaqui = __('Pagaqui', 'eupago-gateway-for-woocommerce');
      ...
      $orders_description = __('Activate only for orders over X € (exclusive). Leave blank (or zero) to allow any order value. The order value must fall within the limits set by Pagaqui.', 'eupago-gateway-for-woocommerce');
      $orders_below = __('Only for orders below', 'eupago-gateway-for-woocommerce');
      $orders_below_description = __('Activate only for orders over X € (exclusive). Leave blank (or zero) to allow any order value. The order value must fall within the limits set by Pagaqui.', 'eupago-gateway-for-woocommerce');
      ...
      $enable_pagaqui = __('Enable Pagaqui', 'eupago-gateway-for-woocommerce');
      ...
      if ($admin_language === 'pt_PT' || $admin_language === 'pt_BR') {
        ...
        $enable_pagaqui = __('Ativar Pagaqui', 'eupago-gateway-for-woocommerce');
        $title_pagaqui = __('Título', 'eupago-gateway-for-woocommerce');
        ...
        $orders_description = __('Ativar apenas para encomendas acima de x € (exclusivo). Deixe em branco (ou zero) para permitir qualquer valor de encomenda.', 'eupago-gateway-for-woocommerce') . ' ' . __('Por definição, o Pagaqui apenas permite pagamentos de 1 a 999999 € (inclusive). Pode usar esta opção para limitar ainda mais este intervalo.', 'eupago-gateway-for-woocommerce');
        ...
        $enable_pagaqui = __('Activar Pagaqui', 'eupago-gateway-for-woocommerce');
        ...
        $orders_description = __('Activar sólo para pedidos superiores a X € (exclusivo). Déjelo en blanco (o cero) para permitir cualquier valor de pedido. El valor del pedido debe estar dentro de los límites establecidos por Pagaqui.', 'eupago-gateway-for-woocommerce');
        $orders_below = __('Solo para pedidos inferiores a', 'eupago-gateway-for-woocommerce');
        $orders_below_description = __('Activar sólo para pedidos superiores a X € (exclusivo). Déjelo en blanco (o cero) para permitir cualquier valor de pedido. El valor del pedido debe estar dentro de los límites establecidos por Pagaqui.', 'eupago-gateway-for-woocommerce');
      ...
      $this->form_fields = [
        'enabled' => [
          ...
          'type' => 'checkbox',
          'default' => 'no',
          'label' => esc_html($enable_pagaqui),
        ],
        'title' => [
          'title' => esc_html($title_pagaqui),
          'type' => 'text',
          'description' => esc_html($controls_checkout),
          'default' => esc_html__($pagaqui),
        ],
        'description' => [
    ...
    public function thankyou_page($order_id)
    {
      ...
      if ($payment_method == $this->id) {
        ...
        wc_get_template('payment-instructions.php', array(
          ...
          'referencia' => $order->get_meta('_eupago_pagaqui_reference', true),
    ...
    function check_order_errors($order_id)
    {
      ...
      if ($order_total >= 1000000) {
        return __('It\'s not possible to use Pagaqui to pay values above 999999&euro;.', 'eupago-gateway-for-woocommerce');
      }
      return false;
    }
    ...
    function process_payment($order_id)
    {
      ...
      if (!is_array($pagaquiResponse)) {
        $logger->error("Invalid API response (not array): " . print_r($pagaquiResponse, true), $context);
        wc_add_notice(__('Erro ao processar a resposta do gateway.', 'eupago-gateway-for-woocommerce'), 'error');
        return ['result' => 'fail'];
      }

      if (isset($pagaquiResponse['transactionStatus']) && strtolower($pagaquiResponse['transactionStatus']) !== 'success') {
        $error_message = $pagaquiResponse['text'] ?? __('Erro desconhecido.', 'eupago-gateway-for-woocommerce');
        $logger->error("Transaction rejected: " . $error_message, $context);
        wc_add_notice(__('Payment error:', 'eupago-gateway-for-woocommerce') . ' ' . $error_message, 'error');
        return ['result' => 'fail'];
      }

      $order->update_meta_data('_eupago_pagaqui_tid', $pagaquiResponse['transactionID'] ?? '');
      $order->update_meta_data('_eupago_pagaqui_reference', $pagaquiResponse['reference'] ?? '');
      $order->save();
